  ...
  return (
    <div className="fixed bottom-8 right-8 z-[60]">
          ...
          >
            <div className="bg-slate-900 p-8 text-white flex justify-between items-center">
              ...
              <button 
                ...
                className="text-white/60 hover:text-white transition-colors p-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white rounded-full" 
            ...
            >
              {messages.map((m, i) => (
                ...
                >
                  <div className={`max-w-[85%] p-6 rounded-[2rem] text-sm leading-relaxed font-medium ${
                    ...
                    <span className="sr-only">{m.role === 'user' ? 'You said:' : 'Amgad AI said:'}</span>
                    {m.text}
                  </div>
                </motion.div>
              ))}
              {isLoading && (
                <div className="flex justify-start">
                  <div className="bg-slate-100 p-6 rounded-[2rem] rounded-bl-none" aria-label="Bot is typing">
            ...
            <form onSubmit={handleSubmit} className="p-8 border-t border-slate-100">
              <div className="flex gap-4">
                <input
                  ...
                  aria-label="Your question to Amgad AI"
                ...
                <button 
                  type="submit"
                  disabled={isLoading}
                  className="bg-slate-900 hover:bg-black text-white p-4 rounded-2xl transition-all disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500"
      ...
      <motion.button
        ...
        className={`w-16 h-16 rounded-full shadow-2xl flex items-center justify-center transition-all duration-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 ${isOpen ? 'bg-slate-900 text-white' : 'bg-indigo-700 text-white shadow-indigo-500/30'}`}
        aria-label={isOpen ? "Close Amgad AI Assistant" : "Open Amgad AI Assistant"}
        aria-expanded={isOpen}
      >
        {isOpen ? <X className="w-8 h-8" /> : <MessageSquare className="w-8 h-8" />}
        {!isOpen && (
          <div className="absolute -top-14 right-0 bg-white border border-slate-100 text-[10px] font-black uppercase tracking-widest px-4 py-3 rounded-2xl shadow-xl text-slate-900 whitespace-nowrap pointer-events-none">
            Ask Amgad's AI
          </div>
        )}
      </motion.button>
